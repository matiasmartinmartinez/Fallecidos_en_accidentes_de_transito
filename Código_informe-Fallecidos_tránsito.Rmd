...
##### Sexo
```{r}
ggplotly(datos %>% count(a,sexo) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(a),fill=sexo))+ labs(x="Sexo",y="Frecuencia",title="Gráfico de Mosaico: frecuencia relativa fallecidos por sexo"))
...
datos %>% ggplot(aes(edad))+ geom_density(aes(fill=a),show.legend = F) + labs(x="Edad",y="Densidad", title="Gráfico de Densidad: Edad del fallecido") + facet_wrap(~a) + theme_gray()
...
### Vehículo
```{r}
datos %>% count(vehi,a) %>% mutate(prop=n/sum(n)) %>% filter(n>=0)  %>% ggplot(aes(reorder(abbreviate(vehi,6),-prop),prop)) + geom_bar(stat="identity") + labs(x="Vehículo",y="Proporción",title= "Gráfico de barras: proporción de tipo de vehículo por año") + theme(plot.title = element_text(size=17,hjust=0.25,colour="lightpink4"))
...
```{r}
datos %>% count(vehi,rol) %>% filter(n>=5) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(abbreviate(vehi,4)),fill=abbreviate(rol,1)))+labs(x="Vehículo",title="Gráfico de mosaico: rol según vehículo", caption="Únicamente fueron tomadas las relaciones con frecuencia absoluta mayor o igual a 5 fallecidos") + guides(fill = guide_legend(title = "Rol")) + theme_minimal() + theme(plot.title = element_text(size=18,colour="aquamarine4"),axis.text.x = element_text(angle=40,size=9))
...
```{r}
datos %>% ggplot() + stat_boxplot(aes(rol,edad,fill=sexo),show.legend = F)+ facet_wrap(~sexo) + labs(x="Rol",y="Edad",title="Diagrama de caja: edad según rol, faceteado por sexo")

datos %>% count(f.dias)%>% mutate(n=n/sum(n)) %>% ggplot() + geom_bar(aes(f.dias,n),stat="identity") +labs(x= "Días de supervivencia luego del accidente" , y= "Proporción" , title="Gráfico de barras: proporción de días de supervivencia") + theme(plot.title = element_text(hjust = 0.4,size=16,colour="gray27"))


datos %>% filter(f.dias<5) %>% count(f.dias)%>% mutate(n=n/sum(n)) %>% ggplot() + geom_bar(aes(f.dias,n),stat="identity") +labs(x= "Días de supervivencia luego del accidente" , y= "Proporción" , title="Gráfico de barras: proporción de días de supervivencia",caption="Únicamente se muestran los días con frecuencia absoluta mayor a 5 fallecidos")+ theme(plot.title = element_text(hjust = 0.4,size=16,colour="gray27"))
...
```{r}
#Un hetmap que cambie a medida de los aos 
#datos %>% separate(fecha,c("dia","mes")) %>% count(dia, mes, a) %>% ggplot( aes(dia,mes)) + geom_tile(aes(fill=n)) + scale_fill_viridis() + facet_wrap(~a)
...
#ndia = format(datos$fecha, format = "%a")
#ndia
