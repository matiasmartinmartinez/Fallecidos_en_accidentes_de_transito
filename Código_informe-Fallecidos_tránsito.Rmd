  ...
  datos %>% ggplot(aes(sexo, edad)) + 
    ...
    theme(plot.title =
            element_text(
              size = 16,
              colour = "grey19")) +
    scale_fill_manual(values = c("yellow2", "springgreen2")) ) 
...
### Vehículo
```{r,echo=F,fig.cap="Gráfico de barras: proporción de fallecidos por vehículo"}
...
# reorder datos %>% dplyr::count(vehi, rol) %>% filter(vehi!="PEATON") %>%    mutate( vehi= ifelse ( n<70, "OTROS", vehi ))%>% group_by(vehi) %>% mutate(prop=n/sum(n)) %>%filter(rol=="CONDUCTOR") 

ggplotly(
  datos %>% dplyr::count(vehi, rol) %>% filter(vehi!="PEATON") %>%
    mutate( vehi= ifelse ( n<70, "OTROS", vehi )) %>% ggplot() +
    geom_mosaic(aes(
      weight = n,
