...
##### Sexo
```{r}
ggplotly(datos %>% count(a,sexo) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(sexo,a),fill=sexo))+ labs(x="Sexo",y="Frecuencia",title="Gráfico de Mosaico: frecuencia relativa fallecidos por sexo"))
...
datos %>% ggplot(aes(edad))+ geom_density(aes(fill=a),show.legend = F) + labs(x="Edad",y="Densidad", title="Gráfico de Densidad: proporcion de edad segun rol") + facet_wrap(~rol) + theme_gray()
```
...
### Vehículo
```{r}
datos %>% count(vehi,a) %>% filter(n>=4) %>% mutate(prop=n/sum(n)) %>% ggplot(aes(reorder(abbreviate(vehi,6),-prop),prop)) + geom_bar(stat="identity") + facet_wrap(~a) + coord_flip() + labs(x="Vehículo",y="Proporción",title= "Proporción de vehículos por año")
```
...
datos %>% count(vehi,otro.vehi) %>% filter(n>=50) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(vehi),fill=otro.vehi))+ labs(x="Vehículo", y="Frecuencia Relativa",title="Grafico de Mosaico: Relación entre los vehículos involucrados",caption=" Únicamente incluye las rerlaciones con frecuencia absoluta mayores o iguales a 50 fallecidos ") + guides(fill = guide_legend(title = "Otro vehículo")) + scale_fill_brewer(palette = "Dark2")
```
...
### Jurisdicción
```{r}
datos %>% count(jur) %>% mutate(prop=n/sum(n)) %>% ggplot() + geom_bar(stat="identity",aes(jur,prop,fill=jur),show.legend = F) + labs(x="Jurisdicción",y="Proporción",title="Grafica de Barra: Proporción de la jurisdicción") + scale_fill_manual(values=c("seagreen3","indianred3"))
