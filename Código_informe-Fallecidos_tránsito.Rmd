...
cbind((datos %>% count(a,dep) %>% arrange(a)),censo=rep(censo))%>% mutate(tasa=(n/censo)*100000) %>% ggplot(
   aes (reorder(abbreviate(dep,4),+tasa),tasa)) +
  geom_bar(stat="identity") +
  labs(
    x="Departamento",
    y="Frecuencia absoluta de fallecidos",
    caption="La recta roja corresponde al promedio de tasa de fallecidos cada 100 mil para el total del país") +
  coord_flip() + 
  theme_minimal() + 
  theme(
    axis.title  =
