...
### Jurisdicción
```{r}
datos %>% count(jur) %>% mutate(prop=n/sum(n)) %>% ggplot() + geom_bar(stat="identity",aes(jur,prop)) + labs(x="Jurisdicción",y="Proporción") 
```
...
datos %>% ggplot() + geom_density(aes(f.dias)) +labs(x="Dias de supervivencia luego del accidente",y="Densidad")
```
