...
paquetes.a.utilizar<- c("tidyverse", "ggmosaic", "plotly", "ggmap", "raster", "rgdal","knitr","zoo")
...
#ggplotly(datos %>% separate(fecha,c("dia","mes"))%>%  mutate(a.mes=as.yearmon(paste(a, " ", mes), format = "%Y %m") ) %>% count(a.mes)%>% arrange(a.mes) %>% mutate(a.mes=as.character(a.mes)) %>% ggplot(aes (a.mes,n,group=0))+ stat_summary(fun.y=sum,geom="line"))

f %>% count(mes) %>% mutate(mes=as.numeric(mes),n.mes=month.abb[as.numeric(mes)])%>% arrange(mes)%>%
     ggplot(  aes(mes,n) ) + 
   ...
   theme_minimal() +
     theme(
       ...
       axis.title = element_text(size=13.6,colour="grey23")) + coord_cartesian(ylim = c(0,1,300))
