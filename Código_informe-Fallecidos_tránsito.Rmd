...
library(plotly)
graf=datos %>% count(dep,a) %>% ggplot(aes(reorder(abbreviate(dep),-n),n,group=a)) + geom_bar(stat="identity")+ labs(x="Departamento",y="Frecuencia acumulada",title="Gráfico de barras: frecuencia acumulada por departamento") +theme(plot.title = element_text(hjust = 0.5,colour="palegreen4",size=15,face="italic"))

ggplotly(graf)%>% animation_opts( 800, redraw = TRUE)
