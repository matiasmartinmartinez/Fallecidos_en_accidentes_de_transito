...
datos %>% filter(f.dias==0) %>% count(tipo.sin)%>% mutate(n=n/sum(n)) %>% ggplot() + geom_bar(aes(reorder(abbreviate(tipo.sin,6),-n),n),stat="identity") +labs(x= "Tipo de Siniestro" , y= "Proporción" , title="Gráfico de barras: proporción de muerte súbita según tipo de siniestro", caption="Como muerte súbita se hace alución a que la persona fallecio en el acto o en menos de 24 horas.") + theme(plot.title = element_text(size = 15,colour="grey20"))
```
