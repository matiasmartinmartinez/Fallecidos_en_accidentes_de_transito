...
datos %>% count(vehi, a) %>% mutate(prop = n / sum(n)) %>% filter(n >= 0)  %>%
...
ggplotly(
  datos %>% count(vehi, otro.vehi) %>% filter(n >= 50) %>%
...
La mayor proporcion de los accidentes desde el punto de vista del fallecido, no ivolucran otro vehículo, y varía considerablemente dependiendo del vehículo en cuestión.
...
datos %>% count(jur, a) %>% mutate(prop = n / sum(n)) %>%
  ggplot() + 
  geom_bar( 
    stat = "identity",
    aes(a, prop, fill = jur),
...
datos %>% count(involucrado, jur) %>% ggplot() +
  geom_mosaic(aes(
    weight = n,
    x = product(jur),
    fill = involucrado)) +
...
# Gráfico de mosaicos: jurisdicción según tipo de siniestro
 datos %>% count(tipo.sin, jur) %>% filter(n >= 20) %>% ggplot() + geom_mosaic(aes(
   weight = n,
...
# Diagrama de caja: edad según rol, faceteado por sexo
ggplotly(
  datos %>% ggplot() + 
    stat_boxplot( 
      aes(rol, edad, fill = sexo),
      show.legend = F) + 
    facet_wrap( ~ sexo) + 
    labs(
      x = "Rol", 
      y = "Edad", 
      title = "Diagrama de caja: edad según rol, faceteado por sexo") + 
    theme_minimal()+
    theme(plot.title = 
            element_text(size = 17,colour="grey20")))
...
ggplotly(
datos %>% count(f.dias) %>% mutate(n = n / sum(n)) %>% ggplot() +
  geom_bar(aes(f.dias, n), stat ="identity") +
  labs(
...
datos %>% filter(f.dias == 0) %>% count(tipo.sin) %>%
  ...
  scale_fill_manual(
    values=c("sienna3","skyblue4","yellowgreen",
             "firebrick1","mediumseagreen","palevioletred4","yellow2"))
```
