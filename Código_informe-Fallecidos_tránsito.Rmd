...
# Tasa de fallecidos cada 100.000 habitantes.
pob.dep<-cbind((datos %>% count(dep) %>% arrange(dep)),censo)%>% mutate(tasa=(n/censo)*10000)

pob.dep %>% ggplot(
   aes (reorder(abbreviate(dep,4),+tasa),tasa)) +
  geom_bar(stat="identity") +
  labs(
    x="Departamento",
    y="Tasa de fallecidos cada 10 mil habitantes") +
  coord_flip() + 
  theme_minimal() + 
  theme(
    axis.title  =
...
La recta roja corresponde a la media de tasas de fallecidos por deparatamento cada 10 mil habitantes para el período designado.


```{r,echo=F,fig.cap="Grafico de mapas: tasa de fallecidos cada 10 mil habitante por departamento"}
mapeo(cbind((datos %>% count(dep) %>% arrange(dep)),censo)%>% mutate(n=(n/censo)*10000)) +
  labs(fill = "Cantidad",
  x = NULL,
  y = NULL) +
