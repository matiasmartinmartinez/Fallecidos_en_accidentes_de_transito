...
graf=datos %>% count(dep,a) %>% arrange(desc(n)) %>%  ggplot(aes(reorder(abbreviate(dep),-n),n,group=a)) + geom_bar(aes(frame=a),stat="identity")+ labs(x="Departamento",y="Frecuencia acumulada",title="Gráfico de barras: frecuencia acumulada por departamento")
