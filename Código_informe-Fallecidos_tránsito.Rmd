...
datos %>% filter(rol!="PEATON")%>% count(vehi,rol) %>% filter(n>=200) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(abbreviate(rol,2),abbreviate(vehi,1)),fill=abbreviate(rol,2))) 
