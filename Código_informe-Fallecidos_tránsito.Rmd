...
ggplot() + 
  geom_bar( 
    ...
    show.legend = T)  + 
  theme_minimal()  +
  labs(x="Rol",y="Proporción",title="Gráfico de barras apilado: rol por año") + 
  theme_minimal() + 
  guides(fill = guide_legend(title = "Rol:")) +
  theme(
    plot.title = (element_text(
...
Para este período trabajado no parece variar significativamente las proporciones, tiende a mantenerse en el tiempo observando que los peatones y pasajeros representan un 20% y los conductor representan un 60%.
...
```
Se puede percibir en la distribucion de densidad por edad segun el vehiculo donde se encontraba el fallecido, que comenzando de una edad muy temprana las motos tienen un gran crecimiento de fallecidos siendo mayores que los fallecidos en autos, alcanzando su tope a la edad cercana de 25 años para luego presentar un significante decenso; tal asi que a las edades cercana de los 50 años los fallecidos en autos empiezan a tener mas protagonismo siendo que  autos en  las edades de 20 años hasta los 75 no habia presetado cambios signiricativos en la distribucion de la densidad.
...
# Gráfico de barras: frecuencia acumulada por departamento
 ggplot(   (datos %>% count(dep)  ),
  ...
  labs(
    ...
    caption="La línea roja corresponde a la media de la frecuencia acumulada total del el país") +
  coord_flip() + 
  theme_minimal() + 
  theme(
    plot.title =
...
Como era de esperarse, el área metropolitana conformada por los departamentos de Montevideo, Maldonado, Canelones y San José, superan la media de fallecidos en el país, lo que significando que la mayoría de los accidentes fatales de tráficos ocurridos se dieron principalmente en estas zonas anteriormente mencionadas.
