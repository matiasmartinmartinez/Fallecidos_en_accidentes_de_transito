...
```
 ...
 Se percibe una fuerte concentración en edades cercanas a los 25 años, la cual es inalterable al paso de los años para el período dado. Sin embargo, la distribución para aproximadamente 30 a 70 años varía cosiderablemente cambiando de poseer una meseta en el 2013, luego poseer una brusca y cosiderable pendiente decreciente en el año 2014 y retomar lentamente su meseta pero más expandida  llegando a los 70 años con proporciones superiores al 10%.
...
```
 ...
 La distribución de la densidad en la edad según el ro del fallecido cambia abruptamente si existela condicion de peaton o no.
...
# Gráfico de barras: proporción de vehículo por año
datos %>% count(vehi, a) %>% mutate(prop = n / sum(n)) %>% filter(n >= 0)  %>%
  ...
  labs(
    x = "Vehículo",
    y = "Proporción", 
    title = "Gráfico de barras: proporción de vehículo por año") +
  theme_minimal()+
  theme(plot.title = element_text(
    size = 17,
    hjust = 0.35,
...
Del gráfico anterior se puede denotar, que la mayor proporción de fallecidos en el lapso de tiempo trabajado, es dada por el vehículo moto, habiendo asi que casi el 50% de los fallecidos se movilizaban con este vehículo.
...
ggplotly(
  datos %>% count(vehi, rol) %>% filter(n >= 5) %>% ggplot() +
    geom_mosaic(aes(
      weight = n,
...
#INCONVENIENTE: ¿filtrar por distinto a  peatones? Es una obviedad que son peatones, pero aporta la probabilidad de falllecer siendo peatón.
```

Es de destacar que quienes sufren la mayor proporción de muertes por accidentes de tránsito son los conductores, principalmente quienes conducen moto u auto.
