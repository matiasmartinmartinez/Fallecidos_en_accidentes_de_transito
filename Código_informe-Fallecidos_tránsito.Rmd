...
datos %>% ggplot(aes(x=edad))+ geom_density(aes(fill=a)) + labs(x="Edad",y="Densidad", title="Gráfico de Densidad: Edad del fallecido") + facet_wrap(~a) + theme_minimal()
```
...
### Vehículo
```{r}
datos %>% count(vehi,a) %>% mutate(prop=n/sum(n)) %>% ggplot(aes(reorder(vehi,-prop),prop)) + geom_bar(stat="identity") + facet_wrap(~a) + coord_flip() + labs(x="Vehículo",y="Proporción",title= "Proporción de vehículos por año")
```
...
datos %>% count(vehi,otro.vehi) %>% filter(n>=80) %>% ggplot() + geom_mosaic(aes(weight=n,x=product(vehi),fill=otro.vehi))+ labs(x="Vehículo", y="Frecuencia Relativa",title="Grafico de Mosaico:Relacion entre los vehículos involucrados") + guides(fill = guide_legend(title = "Otro Vehículo involucrado")) + theme_minimal()
```
...
### Jurisdicción
```{r}
datos %>% count(jur) %>% mutate(prop=n/sum(n)) %>% ggplot() + geom_bar(stat="identity",aes(jur,prop)) + labs(x="Jurisdicción",y="Proporción",title="Grafica de Barra: Proporción de la jurisdicción") 
```
