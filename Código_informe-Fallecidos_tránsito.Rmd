...
datos %>% count(vehi, a) %>% mutate(prop = n / sum(n)) %>% filter(n >= 0)  %>% ggplot(aes(reorder(abbreviate(vehi, 6), -prop), prop)) + 
  geom_bar(stat ="identity") + 
  labs(
    x = "Vehículo",
    y = "Proporción", 
...
ggplotly(
  datos %>% count(vehi, otro.vehi) %>% filter(n >= 50) %>% ggplot() + 
    geom_mosaic(aes( 
      weight = n,
      x = product(vehi),
      fill = otro.vehi)) +
...
# Gráfico de barras: frecuencia acumulada por departamento
 ggplot( datos %>% count(dep), aes (reorder(abbreviate(dep,4),+n),n)) +
  geom_bar(stat="identity") +
  labs(
    x="Departamento",
  ...
  theme(
    plot.title =
      element_text(
        ...
        size=15,face="bold")) +  
  geom_hline( yintercept = mean((datos %>% count(dep))$n) ,
              cex=1.2,colour="coral3") 
...
datos %>% count(jur, a) %>% mutate(prop = n / sum(n)) %>% ggplot() + 
  geom_bar( 
    stat = "identity",
    aes(a, prop, fill = jur),
...
ggplotly(
  datos %>% ggplot() + stat_boxplot(aes(rol, edad, fill = sexo), show.legend = F) + 
    facet_wrap( ~ sexo) + 
    labs(
      x = "Rol", 
      y = "Edad", 
...
datos %>% filter(f.dias == 0) %>% count(tipo.sin) %>% mutate(n = n / sum(n)) %>% ggplot() +         geom_bar(aes(reorder(abbreviate(tipo.sin, 6), -n), n), stat = "identity") +
       labs(                                                                                            x = "Tipo de Siniestro" ,
                                                                                                        y = "Proporción" ,
  ...
  theme(plot.title = element_text(size = 15, colour = "grey20"))
...
n.dia <-  weekdays(as.Date(
  datos %>% separate(fecha, c("dia", "mes")) %>% dplyr::select(a, mes, dia) %>% unite(sep =
  "-") %>% unlist()))
...
mapeo<-  function(n) 
  ...
  theme_opts <- list(theme(panel.grid.minor = element_blank(),
                         ...
                         plot.title =
                           element_text(size=16.4,colour="darkslategrey",hjust=0.25)))
