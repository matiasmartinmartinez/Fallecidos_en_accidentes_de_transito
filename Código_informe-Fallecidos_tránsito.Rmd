...
f %>% count(mes) %>% arrange(desc(n)) %>% ggplot(aes(mes,n)) + geom_bar(aes(fill=2),stat="identity",show.legend = F) + labs(x="Mes", y="Frecuencia absoluta" ,title="Gráfico de barras: frecuencia absoluta por mes") + theme(plot.title = element_text(size=17,colour="grey30",face="bold",hjust=0.4)) 
