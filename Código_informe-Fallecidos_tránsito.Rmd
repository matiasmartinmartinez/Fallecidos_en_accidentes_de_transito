...
```{r}
fecha<-datos %>% separate(fecha,c("dia","mes")) %>% select(a,mes,dia) %>% unite(sep="-") %>% unlist()
n.dia<-weekdays(as.Date(fecha.ren))
fecha.sep<-datos %>% separate(fecha,c("dia","mes")) %>% select(1:3)

f<-data.frame(fecha,fecha.sep,n.dia)
